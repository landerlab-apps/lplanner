...
### Imperial units did not work

**If you plan in feet, every schedule you produced before this release was
built on metric numbers reinterpreted as imperial ones.** Reported by testers,
and confirmed on measurement.

Switching the units control converted nothing. Every setting was stored as
typed and passed to the engine unchanged, and every shipped default was a
metric number — so choosing Feet did not convert 3 m into 10 ft, it told the
engine the stop grid was 3 **ft**.

| setting | default | read as, in Feet | should have been |
|---|---|---|---|
| Stop distance | 3 | 3 ft (0.9 m) | 10 ft |
| Last stop | 3 | 3 ft (0.9 m) | 10 ft |
| Max END | 40 | 40 ft (12 m) | 130 ft |
| Descent rate | 15 | 15 ft/min | 50 ft/min |
| Ascent bands | 70-30, 30-12, 12-0 | feet | 230-100, 100-40, 40-0 |

The same 40 m / 20 min dive ran 45 minutes in Meters and 54 in Feet, on a stop
ladder of 51/33/27/18/15 ft. The most dangerous consequence was the ascent
table: read as feet, the bands covered nothing above 70 ft, so a 131 ft dive
had **no defined ascent rate over its deepest third**.

Separately, the gas report could not be put into imperial at all. The engine
has always supported cubic feet, but the apps always wrote an explicit
`RmvMetric` line, which disabled the engine's own "follow the depth units"
default. Choosing Feet gave depths in feet and gas in litres; setting the RMV
control as well, without also fixing the numbers, read a 19 L/min RMV as
19 cu.ft/min — about 538 L/min — and reported 2383 cu.ft of air for a 40 m
dive.

**Now:** switching units converts altitude, stop distance, last stop, END, both
rate tables, the deco setpoint depths, the RMVs and your entered dive levels.
Rounding is to whole units, so the round trip is lossless (3 m → 10 ft → 3 m)
and lands on the values divers actually use. Gas units follow the depth units
until you set them yourself. Every Config field now shows the unit it is in.

Verified: the metric reference plan is byte-identical to 1.21.0, and the Meters
and Feet forms of the same dive now agree — 44 vs 43 minutes run time, gas
within 0.5 %, stops on whole feet. Cross-checked against MultiDeco on a 230 ft
trimix dive (18/45, GF 45/85, salt): same first stop, same gas switches, eight
of twelve stop times identical and the rest one minute longer.

### The experimental extra-slow ascent rule is removed

It never did anything, for two independent reasons, both measured.

The rule was documented as holding the ascent while supersaturation projected
at the depth being ascended *to* exceeded 1.25 bar. The code accepted that
target depth and discarded it, measuring at the depth the diver was already at
— which, since tissue tensions do not depend on depth, is lower by exactly the
ambient-pressure gap, about 0.30 bar on a 3 m or 10 ft grid, or 24 % of the
threshold.

More fundamentally, an absolute 1.25 bar trigger is incompatible with gradient
factors, whose whole purpose is to cap supersaturation. Peak gradient measured
across seven dives from 30 m on air to 100 m on 10/70, at realistic gradient
factors: 0.43 to 1.17 bar. The rule only ever engaged at GF 100/100 — the raw
Bühlmann ceiling. Fixing the first defect would not have changed that.

No schedules change as a result: every bundled sample produces byte-identical
output. That is the argument for removing it. `ExtraSlow:` is still accepted in
a `profile.dat` and ignored, so existing files keep working.

### Also

The profile parser now finds `UseMetric` and `RmvMetric` wherever they sit in
the file. Previously the unit scaling was applied as each key was read, so any
value written above `UseMetric` was converted with the imperial default,
silently — a trap for hand-written or third-party profiles.
...
The section was rewritten from the decompression literature rather than from
